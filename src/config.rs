...
pub fn save_history(history: &[crate::engine::HistoryEntry]) {
    if !get().history.auto_save {
        return;
    }
    let _ = fs::create_dir_all(dir());
...
pub fn load_history() -> Vec<crate::engine::HistoryEntry> {
    if !get().history.auto_save {
        return Vec::new();
    }
    let p = history_path();
...
pub fn export_history_json(history: &[crate::engine::HistoryEntry]) -> PathBuf {
    let _ = fs::create_dir_all(dir());
    let p = dir().join("history_export.json");
...
pub fn export_history_csv(history: &[crate::engine::HistoryEntry]) -> PathBuf {
    let _ = fs::create_dir_all(dir());
